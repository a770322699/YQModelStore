...
// 返回一个单例对象
+ (YQModelStore *)modelStoreWithIdentity:(NSInteger)identity{
...
- (BOOL)persistenStoreCoordinator:(NSPersistentStoreCoordinator *)coordinator addPersistentStore:(NSURL *)dataBasePath{
    BOOL dataBaseExit = [[NSFileManager defaultManager] fileExistsAtPath:dataBasePath.absoluteString];
    BOOL success = [coordinator addPersistentStoreWithType:NSSQLiteStoreType
                                             configuration:nil
                                                       URL:dataBasePath
                                                   options:_option
                                                     error:NULL];
    if (!success && dataBaseExit) {
        BOOL removed = [[NSFileManager defaultManager] removeItemAtPath:dataBasePath.absoluteString error:NULL];
        if (removed) {
